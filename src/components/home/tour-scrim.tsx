...
function sideGradient(direction: "right" | "left") {
  const a = (fraction: number) => `rgba(4,6,14,${(PEAK * fraction).toFixed(3)})`;
  return [
...
/** Flat cover: carries mobile, and sinks the CTA's burned-in headline. */
const FLAT = `rgba(4,6,14,${(PEAK * 1.067).toFixed(3)})`;
 ...
 */
const EDGES =
  "linear-gradient(to bottom, rgba(4,6,14,0.55) 0, rgba(4,6,14,0) 18%, rgba(4,6,14,0) 70%, rgba(4,6,14,0.45) 100%)";
